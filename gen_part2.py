#!/usr/bin/env python3
"""Part 2: Template function for city pages"""

def generate_city_html(c, price_table):
    city = c["city"]
    slug = c["slug"]
    zona = c["zona"]
    price = c["price"]
    ...
    city_encoded = city.replace(" ", "%20").replace("â", "â").replace("ã", "ã").replace("ç", "ç").replace("é", "é").replace("ô", "ô")
    
    spec_items_html = "\n".join(f"<li>{item}</li>" for item in c["spec_items"])
    prob_items_html = "\n".join(f"<li><strong>{item.split(' ', 1)[0]}</strong> {item.split(' ', 1)[1] if ' ' in item else ''}</li>" for item in c["problems"])
...
<meta name="keywords" content="eletricista {city.lower()}, eletricista urgente {city.lower()}, eletricista 24 horas, quadro elétrico, certificação elétrica, {city.lower()}, trás-os-montes" />
...
<body>
<a href="https://wa.me/351932321892?text=Olá,%20preciso%20eletricista%20{city_encoded}" class="whatsapp-float" aria-label="WhatsApp">💬</a>
...
⚡ <strong>Desde Macedo:</strong> {km} km ({mn} minutos) | 🏛️ <strong>Zonas:</strong> {zones}
</p>
</div>
...
<h2 style="margin:0 0 15px">📞 URGÊNCIA EM {city_upper}?</h2>
...
<p style="font-size:14px;color:#666;margin-top:10px">
ou <a href="https://wa.me/351932321892?text=Olá,%20preciso%20eletricista%20{city_encoded}" style="color:#25D366;font-weight:bold;text-decoration:none">WhatsApp 💬</a>
</p>
</div>
...
<p style="font-size:18px;line-height:1.8">
Precisa de <strong>eletricista urgente em {city}</strong>? Chego em <strong>{mn} minutos</strong> desde Macedo de Cavaleiros com todo o equipamento profissional necessário.
</p>

<div class="local-section">
<h3>{c["spec_title"]}</h3>
<p style="font-size:16px;line-height:1.8;color:#555">{c["spec_desc"]}</p>
...
<li>✅ Profissional certificado baseado em Macedo ({km}km)</li>
<li>✅ <strong>Equipamento profissional Ridgid &amp; Fluke</strong></li>
<li>✅ Chego em {mn} minutos em {city}</li>
...
<p style="font-size:16px;margin:0 0 10px">"{c["r1"]}"</p>
<p style="font-size:14px;color:#999;margin:0">— {c["r1n"]}</p>
</div>
<div style="background:white;padding:20px;margin:15px 0;border-radius:8px;border-left:4px solid #FFD700">
<div style="color:#FFD700;font-size:18px;margin-bottom:10px">⭐⭐⭐⭐⭐</div>
<p style="font-size:16px;margin:0 0 10px">"{c["r2"]}"</p>
<p style="font-size:14px;color:#999;margin:0">— {c["r2n"]}</p>
</div>
</div>
...
<br><br>
<a href="https://wa.me/351932321892?text=Olá,%20preciso%20eletricista%20{city_encoded}" style="display:inline-block;background:#25D366;color:white;padding:20px 40px;border-radius:50px;font-size:24px;font-weight:bold;text-decoration:none;margin:10px;box-shadow:0 4px 15px rgba(37,211,102,0.3)">💬 WhatsApp</a>
</div>
</section>
